...
CAmount CMasternode::GetBlockValue(int nHeight)
{
     ...
     if (nHeight >   6000000) return     0.0 * COIN;
    if (nHeight >   5000000) return     0.16 * COIN;
    if (nHeight >   4000000) return     0.20 * COIN;
    if (nHeight >   3000000) return     0.25 * COIN;
    if (nHeight >   2000000) return     0.31 * COIN;
    if (nHeight >   1000000) return     0.625 * COIN;
    if (nHeight >    500000) return     1.25 * COIN;
    if (nHeight >    400000) return     2.5 * COIN;
    if (nHeight >    300000) return     5  * COIN;
    if (nHeight >    200000) return     10 * COIN;
    if (nHeight >         2) return     20 * COIN;
    if (nHeight >         1) return     106000000 * COIN;
...
bool CMasternodeBroadcast::CheckInputsAndAdd(int& nDoS)
{
    ...
    // should be at least not earlier than block when 1000 APH tx got MASTERNODE_MIN_CONFIRMATIONS
    ...
    if (mi != mapBlockIndex.end() && (*mi).second) {
        ...
        int nConfHeight = pMNIndex->nHeight + MASTERNODE_MIN_CONFIRMATIONS - 1; // block for 1000 APH tx -> 1 confirmation
