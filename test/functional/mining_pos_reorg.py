...
class ReorgStakeTest(PivxTestFramework):
    ...
    def check_money_supply(self, expected_piv, expected_zpiv):
        ...
        # verify that nodes have the expected APH and zAPH supply
        for node in g_info:
            assert_equal(node['moneysupply'], DecimalAmt(expected_piv))
            for denom in node['zAPHsupply']:
                assert_equal(node['zAPHsupply'][denom], DecimalAmt(expected_zpiv[denom]))
    ...
    def run_test(self):
        ...
        # Check APH and zAPH supply at the beginning
        # ------------------------------------------
        # zAPH supply: 2 coins for each denomination
        ...
        # APH supply: block rewards minus burned fees for minting
        ...
        # Verify that APH and zAPH supplies were properly updated after the spends and reorgs
        self.log.info("Check APH and zAPH supply...")
